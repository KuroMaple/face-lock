...
export default function Lock({locked, canPlaySound}: {locked: boolean, canPlaySound: boolean}) {

  const clickSound = useMemo(() => (typeof window !== "undefined" ? new Audio('/sounds/click.wav') : null), []);
  const sciFiSound = useMemo(() => (typeof window !== "undefined" ? new Audio('/sounds/SciFiSound.wav') : null), []);


  useEffect(() => {
    const audio = locked ? clickSound : sciFiSound;
    if (!audio) return; // Prevent errors on serve
    audio.currentTime = 0;
